...
{
    public class DragonOffsetCalculator : IOffsetCalculator
	{
		...
		/// <summary>
		/// Factory for creating the event log readers.
		/// </summary>
		private EventLogFactory EFactory;
		...
		public DragonOffsetCalculator(List<IMatch<string>> matches)
		{
			if (matches == null)
			{
			...
			foreach (IMatch<string> match in matches)
			{
                ...
                try
                {
                    match.SelectedItems().Single(file => Path.GetExtension(file) == DRAGON_EXT);
                }
                catch (Exception)
                {
                    throw new ArgumentException("A match for merging must contain exactly one dragon (" + DRAGON_EXT + ") file. Make sure" +
                    " that all matches contain one dragon (" + DRAGON_EXT + ") file.");
                }
			}
