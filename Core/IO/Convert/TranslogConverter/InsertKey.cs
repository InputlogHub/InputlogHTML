...
{
    /// <summary>
    /// Converting a Translog insert key event.
    /// </summary>
    public class InsertKey : ILConvertible
    {
        #region Fields
        ...
        public InsertKey(XPathNavigator node, TranslogReader.DocParam param)
        {
            Node = node;
            Parameter = param;
        ...
        public IEnumerable<Event> ConvertToEvent()
        {
			...
			if (text.Length > 0)
			{
			    ...
			    var thisKey = value.Equals(" ") ? KeysEx.VK_SPACE : KeysEx.NONE;
				...
				Parameter.Position += value.Length;
				Parameter.DocLength = Parameter.DocLength - text.Length + value.Length;

				thisEvent = new Event();
				thisEvent.Properties["type"] = "replacement";
				thisEvent.Properties["id"] = (Parameter.Id++).ToString();
				thisEvent.Parts.Add(new Replacement(cursor, cursor + text.Length, value));
				EventList.Add(thisEvent);
