...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/">
    <html>
      <head>
        <title>Word Pause Analysis</title>
        <link rel="stylesheet" type="text/css" href="Style/common.css" />
      </head>
      <body>
        <!-- Header and sessionidentification -->
        <xsl:call-template name="header" />
        <h1>Word Pauses Analysis File</h1>
        ...
        <table cellspacing="0" cellpadding="5" border="0" width="100%" class="content">
          ...
          <tbody>
            <xsl:for-each select="session/wordPauses/infoTable">
              <tr>
                <!-- 0 Revisions -->
                <td>
                ...
                <td align="right">
                  <xsl:choose>
                    <xsl:when test="not(normalize-space(Target)='')">
