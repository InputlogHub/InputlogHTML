...
$(document).ready(function () {
    ...
    /*Fires when the user depresses a key while in input area.
    It repeats while the user keeps the key depressed.*/
    $("#inputarea").keydown(function (event) {
        if(isFirstKeyPress){
            window.startTimer();       //function in tasks.html
         ...
         * been passed yet.
         */
       if(window.ffa_repetition && event.which === 13){
            this.rows += 1;
        } else {
            /* 
            ...
            if(this.value.lineCount() >= maxRows && 
                    event.which === 13) {
                const textAreaContentLines = this.value.lines();
        ...
        const myChar = specialCharacter(event.keyCode, event.Key);
        const keyCode = myChar.key;
        const keyValue = myChar.value;
        ...
        const time = window.performance.now() - initialDowntime;

        //document length and position
        const area = $("#inputarea")[0];
        const docLength = area.value.length;
        const cursorPosition = $("#inputarea").getCursorPosition();

        logEvent = {
            keyCode: keyCode, keyValue: keyValue, downtime: time, type: "keyEvent",
            position: cursorPosition, docLength: docLength
        };
    ...
    $("#inputarea").keypress(function (event) {
        ...
        const keyValue2 = String.fromCharCode(event.keyCode);

        const currEvent = events[events.length - 1];
        if (currEvent.keyValue == null && currEvent.keyCode != null) {
            //this means the key was a special character, but the key value was not known at the time of the keydown event
    ...
    $("#inputarea").keyup(function (event) {
        ...
        window.setAdvanceAvailability();
    });

    function specialCharacter(charCode, keyValue) {

        window.standard_mapping = {
        ...
        if (!(charCode in window.standard_mapping)) {
            return { key: null, value: null };
        }

        const result = window.standard_mapping[charCode];
...
function showOutput() {
    ...
    for (var i = 0; i < events.length; i++) {
        const myEvent = events[i];
        window.message += toHtml(myEvent);
    }
    $("#output").html(window.message);
}
...
function toHtml(myEvent) {
    ...
    if (myEvent.type === "keyEvent") {
        window.message = '<tr align="center"><td>' + myEvent.keyValue + " (code " + myEvent.keyCode + ")" + '<td style="width:200px"> starttime '
            + Math.round(myEvent.downtime * 100) / 100 + '<td style="width:200px"> endtime ' + Math.round(myEvent.uptime * 100) / 100 + " </tr>";
    } else if (myEvent.type === "focusEvent") {
        window.message = '<tr align="center"><td>' + myEvent.title + '<td style="width:200px"> starttime ' + Math.round(myEvent.downtime * 100) / 100
...
function entriesToXML(entries) {
    ...
    for (var key in entries) {
        if (Object.prototype.hasOwnProperty.call(entries, key)) {
            window.exml += "\t\t" + openElement("entry");
            window.exml += "\t\t\t" + createElement("key", key);
            window.exml += "\t\t\t" + createElement("value", entries[key]);
            window.exml += "\t\t" + closeElement("entry");
        }
    }
    return window.exml;
}
...
function getCurrentDate() {
    ...
    const sep = "-";
    const todayString = dd + sep + mm + sep + yyyy;
    return todayString;
}

function eventsToXML(events, doNotShowQuestions)
{
    window.xml_string = "";
    window.xml_string += '<?xml version="1.0" encoding="utf-8"?>' + crlf();
    window.xml_string += openElement("log");
    var fileName = sessionObject.getParticipant() + "_" + window.copyTaskName + "_" + getCurrentDate() + ".idfx";

    window.xml_string += "\t" + openElement("meta");
    var meta = {
        ...
        __copytask: window.activeCopyTaskContent
    };
    window.xml_string += entriesToXML(meta);
    window.xml_string += "\t" + closeElement("meta");

    window.xml_string += "\t" + openElement("session");
    var session = {
    ...
    window.xml_string += entriesToXML(session);
    window.xml_string += "\t" + closeElement("session");
    ...
    for (i = 0; i < events.length; i++) {
        ...
        if (myEvent.type === "keyEvent") {
            window.xml_string += '\t<event type="keyboard" id="' + i + '">' + crlf();

            window.xml_string += '\t\t<part type="wordlog">' + crlf();
            window.xml_string += "\t\t\t" + createElement("position", myEvent.position);
            window.xml_string += "\t\t\t" + createElement("documentLength", myEvent.docLength);
            window.xml_string += "\t\t\t" + createElement("replay", "True");
            window.xml_string += "\t\t</part>" + crlf();

            window.xml_string += '\t\t<part type="winlog">' + crlf();

            window.xml_string += "\t\t\t" + createElement("startTime", myToFixed(myEvent.downtime, myEvent.uptime));
            window.xml_string += "\t\t\t" + createElement("endTime", myToFixed(myEvent.uptime, myEvent.downtime));
            ...
            window.xml_string += "\t\t\t" + createElement("key", myKey);
            window.xml_string += "\t\t\t" + createElement("value", myValue);
            window.xml_string += "\t\t\t<keyboardstate/>" + crlf();
            window.xml_string += "\t\t</part>" + crlf();
            window.xml_string += "\t</event>" + crlf();
        }
        else if (myEvent.type === "focusEvent") {
            window.xml_string += '\t<event type="focus" id="' + i + '">' + crlf();
            window.xml_string += '\t\t<part type="winlog">' + crlf();

            window.xml_string += "\t\t\t" + createElement("title", myEvent.title);
            window.xml_string += "\t\t\t" + createElement("startTime", myToFixed(myEvent.downtime, myEvent.uptime));
            window.xml_string += "\t\t\t" + createElement("endTime", myToFixed(myEvent.uptime, myEvent.downtime));
    ...
    // Skip the final questions if such was the setting in the copyTask.
    if (doNotShowQuestions === false) {
        writeQuestions();
    }

    // Save questionnaire in a final event. 
    function writeQuestions() {
        window.xml_string += '\t<event type="questions" id="' + i + '">' + crlf();
        window.xml_string += '\t\t<part type="winlog">' + crlf();
        window.xml_string += "\t\t\t" + createElement("handedness", qAnswers.get_handedness_score().toString());
        window.xml_string += "\t\t\t" + createElement("computer", qAnswers.get_computer());
        window.xml_string += "\t\t\t" + createElement("keyboard", qAnswers.get_keyboard_familiarity());
        window.xml_string += "\t\t\t" + createElement("browser", qAnswers.get_browser());
        window.xml_string += "\t\t\t" + createElement("language", qAnswers.get_languages());
        window.xml_string += "\t\t\t" + createElement("disorder", qAnswers.get_disorder().toString());
        window.xml_string += "\t\t\t" + createElement("education", qAnswers.get_education());
        window.xml_string += "\t\t\t" + createElement("repetition", qAnswers.get_repetition().toString());
        window.xml_string += "\t\t</part>" + crlf();
        window.xml_string += "\t</event>" + crlf();

    }

    window.xml_string += closeElement("log");

    return window.xml_string;
}
